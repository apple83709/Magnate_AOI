...
# direction = 'N'
    ...
    for loop in range(1, 7):
        ...
        # =============================================================================
        # 
        # # Warp target image to align with source image
        # 
        # =============================================================================
        ...
        # Warp target image to align with source image
        image_dst_aligned = cv2.warpPerspective(image_dst, H_inv, (image_src.shape[1], image_src.shape[0]))
        
        # plt.imshow(image_dst_aligned,cmap='gray')
        # plt.show()
        ...
        np_dst = np.zeros([len(df1),32,32]).astype(np.uint8)
        
        
        image_dst_color = color_image = cv2.cvtColor(image_dst_aligned, cv2.COLOR_GRAY2BGR)
        ...
        # print('np_dst shape:' + str(np_dst.size))
        np.save(out_NpFile, np_dst)
